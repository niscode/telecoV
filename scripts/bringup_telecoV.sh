...
tmux split-window -v
tmux split-window -v
tmux send-keys -t $session:$base_window_name.0 "roslaunch telecoV navigation_teb_diff.launch rover_type:=petra start_rviz:=false map_file:=atr_2fw.yaml" Enter
sleep 5
...
tmux send-keys -t $session:$capf_window_name.1 "roscd telecoV/rviz ; rviz -d sim.rviz" Enter
